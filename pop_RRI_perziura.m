% r_RRI_perziura - EKG ir R-R intervalų peržiūra, RRI koregavimas
% Tai „Ritminukui“ adaptuota „pop_RRI_perziura“ versija iš EEG Darbelių.
% 
...
%
% (C) 2014-2023 Mindaugas Baranauskas
...
gui_State = struct('gui_Name',       mfilename, ...
    ...
    'gui_OpeningFcn', @r_RRI_perziura_OpeningFcn, ...
    'gui_OutputFcn',  @r_RRI_perziura_OutputFcn, ...
    'gui_LayoutFcn',  [] , ...
    'gui_Callback',   []);
...
function r_RRI_perziura_OpeningFcn(hObject, eventdata, handles, varargin)
...
% Importuoti_laikus
h15 = uimenu( 'Parent',h14, 'Accelerator','L', 'Callback',@(hObject,eventdata)pop_RRI_perziura('Importuoti_laikus_Callback',hObject,eventdata,guidata(hObject)), 'Label','R laikus iš TXT', 'Tag','Importuoti_laikus');
% Importuoti_RRI
...
% EKG_is_TXT
h17 = uimenu( 'Parent',h14, 'Callback',@(hObject,eventdata)pop_RRI_perziura('Importuoti_EKG_is_txt_Callback',hObject,eventdata,guidata(hObject)), 'Label','Kardiogramą iš TXT', 'Separator','off', 'Tag','EKG_is_TXT');
% Importuoti_KTU_MAT
%h28 = uimenu( 'Parent',h14, 'Callback',@(hObject,eventdata)pop_RRI_perziura('Importuoti_KTU_MAT_Callback',hObject,eventdata,guidata(hObject)), 'Label','KTU *VNS_*.MAT', 'Tag','Importuoti_KTU_MAT');
h28 = uimenu( 'Parent',h14, 'Callback',@(hObject,eventdata)pop_RRI_perziura('Importuoti_Ritminukas_RRT_Callback',hObject,eventdata,guidata(hObject)), 'Label','R laikus ir EKG iš RRT (Ritminuko)', 'Tag','Importuoti_KTU_MAT', 'Separator','off');
...
% Eksportuoti_RRI
h23 = uimenu( 'Parent',handles.m_Eksportuoti, 'Callback',@(hObject,eventdata)pop_RRI_perziura('Eksportuoti_RRI_Callback',hObject,eventdata,guidata(hObject)), 'Label','R-R intervalus', 'Tag','Eksportuoti_RRI');
% Eksportuoti_laikus
...
% Eksportuoti_laikus_KTU
h29 = uimenu( 'Parent',handles.m_Eksportuoti, 'Callback',@(hObject,eventdata)pop_RRI_perziura('Eksportuoti_laikus_Ritminukas_Callback',hObject,eventdata,guidata(hObject)), 'Label','R laikus ir EKG į RRT (Ritminukas)', 'Tag','Eksportuoti_laikus');
...
% EKG rodymas
handles.m_ekg_rodyti = uimenu( 'Parent',handles.m_Rodymas, 'Callback',@(hObject,eventdata)pop_RRI_perziura('checkbox_ekg_Callback',hObject,eventdata,guidata(hObject)), 'separator', 'on', 'checked','on', 'Label',lokaliz('Show EKG'), 'Tag','ekg_rodyti');
% Nejungti
...
function varargout = r_RRI_perziura_OutputFcn(hObject, eventdata, handles)
...
% --------------------------------------------------------------------
function Eksportuoti_laikus_KTU_Callback(hObject, eventdata, handles)
% hObject    handle to Eksportuoti_laikus (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
Eksportuoti_Callback(hObject, eventdata, handles, 'R laikai -> *.MAT');

% --------------------------------------------------------------------
function Eksportuoti_laikus_Ritminukas_Callback(hObject, eventdata, handles)
% hObject    handle to Eksportuoti_laikus (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
Eksportuoti_Callback(hObject, eventdata, handles, 'R laikai -> *.RRT (Ritminukas)');
...
% --------------------------------------------------------------------
function Importuoti_KTU_MAT_VNS_Callback(hObject, eventdata, handles, varargin)
% hObject    handle to Importuoti_laikus (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
susaldyk(hObject, eventdata, handles);
if nargin < 4;
    [f,p]=uigetfile({'*VNS_*.mat'; '*VNS_*.MAT'; '*.*' ;});
    if or(isempty(f),f==0) ;
        susildyk(hObject, eventdata, handles);
        return ;
    end;
...
try
    fprintf('Įkeliama %s\n',rinkmena);
    VNS_duomenys=load(rinkmena);
    EKG_Hz=VNS_duomenys.Data_ECG.Fs_ecg;
    disp('Filtruojama naudojant fun_ECG_Filter...');
    EKG=fun_ECG_Filter(EKG_Hz,VNS_duomenys.Data_ECG.ECG_II');
    handles.EKG=EKG;
    handles.EKG_laikai=VNS_duomenys.Data_ECG.Time_ECG';
    handles.EKG_Hz=EKG_Hz;
    handles.zmkl_lks=[]; handles.zmkl_pvd={};
    disp('Ieškoma R...');
    try
        R_indeksai=fun_qrsDetector(EKG, EKG_Hz*0.4, EKG_Hz);
        Laikai=1000*VNS_duomenys.Data_ECG.Time_ECG(R_indeksai)';
        set(handles.axes_rri,'UserData',Laikai);
    catch
...
% --------------------------------------------------------------------
function Importuoti_Ritminukas_RRT_Callback(hObject, eventdata, handles, varargin)
% hObject    handle to Importuoti_laikus (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
susaldyk(hObject, eventdata, handles);
if nargin < 4;
    [f,p]=uigetfile({'*.rrt'; '*.RRT'; '*.*' ;});
    if or(isempty(f),f==0) ;
        susildyk(hObject, eventdata, handles);
        return ;
    end;
...
try
    fprintf('Įkeliama %s\n',rinkmena);
    MAT_kintamieji = who('-file', rinkmena);
    if ismember({'R_laikai_taisyti'},MAT_kintamieji)
        load(rinkmena,'-mat','R_laikai_taisyti');
        set(handles.axes_rri,'UserData',1000*R_laikai_taisyti);
    else
        error('Nerastas R_laikai_taisyti kintamasis rinkmenoje %s', rinkmena)
    end
    if ismember({'EKG'},MAT_kintamieji)
        load(rinkmena,'-mat','EKG');
        if ~isempty(EKG) && isa(EKG,'timeseries')
            handles.EKG=EKG.Data;
            handles.EKG_laikai=EKG.Time;
            handles.EKG_Hz=1/median(diff(EKG.Time));
        end
    end
    handles.zmkl_lks=[]; handles.zmkl_pvd={};
...
% --------------------------------------------------------------------
function Importuoti_KTU_MAT_AuriMod_Callback(hObject, eventdata, handles, varargin)
% hObject    handle to Importuoti_laikus (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
susaldyk(hObject, eventdata, handles);
if nargin < 4;
    [f,p]=uigetfile({'AuriMod*.mat'; 'AuriMod*.MAT'; '*.*' ;});
    if or(isempty(f),f==0) ;
        susildyk(hObject, eventdata, handles);
        return ;
    end;
...
try
    fprintf('Įkeliama %s\n',rinkmena);
    AuriMod_duomenys=load(rinkmena);
    disp('Filtruojama naudojant fun_ECG_Filter...');
    leads=regexp(fieldnames(AuriMod_duomenys),'ECG_Lead_I+','match'); leads=[leads{:}];
    if isempty(leads)
        error(lokaliz('No channels found.'))
    elseif length(leads) == 1
        KanaloNr=1;
    else
        Kanalai=leads;
        KanaloNr=listdlg(...
    ...
    EKG_Hz=AuriMod_duomenys.Fs_ecg;
    EKG=fun_ECG_Filter(EKG_Hz,AuriMod_duomenys.(leads{KanaloNr})');
    handles.EKG=EKG;
    handles.EKG_laikai=AuriMod_duomenys.Time_ECG';
    handles.EKG_Hz=EKG_Hz;
    handles.zmkl_lks=[]; handles.zmkl_pvd={};
    disp('Ieškoma R...');
    try
        R_indeksai=fun_qrsDetector(EKG, EKG_Hz*0.4, EKG_Hz);
        Laikai=1000*AuriMod_duomenys.Time_ECG(R_indeksai)';
        set(handles.axes_rri,'UserData',Laikai);
    catch
        try   Aptikti_EKG_QRS_Callback2(hObject, eventdata, handles);
        catch err; Pranesk_apie_klaida(err, 'EKG QRS aptikimas', rinkmena, 0);
...
% --------------------------------------------------------------------
function Importuoti_KTU_MAT_DIA_Callback(hObject, eventdata, handles, varargin)
% hObject    handle to Importuoti_laikus (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
susaldyk(hObject, eventdata, handles);
if nargin < 4
    [f,p]=uigetfile({'DIA_*.mat'; 'DIA_*.MAT'; '*.*' ;});
    if or(isempty(f),f==0)
        susildyk(hObject, eventdata, handles);
        return ;
    end
...
try
    fprintf('Įkeliama %s\n',rinkmena);
    DIA_duomenys=load(rinkmena);
    EKG_Hz=DIA_duomenys.Data.Fs_ecg;
    Kanalu_N=size(DIA_duomenys.Data.ECG,2);
    if Kanalu_N > 1
        EEG.nbchan=Kanalu_N;
        EEG.chanlocs=[];
        EEG.pnts=size(DIA_duomenys.Data.ECG,1);
        EEG.data=DIA_duomenys.Data.ECG'; for i=1:Kanalu_N; EEG.data(i,:)=EEG.data(i,:)/rms(EEG.data(i,1:min(EKG_Hz*10,end)))*1000; end
        EEG.srate=EKG_Hz;
        EEG.times=DIA_duomenys.Data.Time_ECG*1000;
        EEG.xmin=EEG.times(1);
        EEG.xmax=EEG.times(2);
        [~,EEG.setname]=fileparts(rinkmena);
        EEG.icawinv=[]; EEG.icaweights=[]; EEG.icasphere=[]; EEG.icaact=[];
        EEG.trials=1;
        EEG.event=[];
        %EEG=eeg_checkset(EEG);
        pop_eeg_perziura(EEG,'laukti',1,'plotis',30);
    ...
    elseif Kanalu_N == 1
        ...
        error('size(Data.ECG)=0');
    end
    
    EKG=DIA_duomenys.Data.ECG(:,KanaloNr)';
    %if ekg_apversta(EKG,EKG_Hz,0); EKG=-EKG; end
    if exist('fun_ECG_Filter','file')
        disp('Filtruojama naudojant fun_ECG_Filter...');
        EKG=fun_ECG_Filter(EKG_Hz,EKG);
    end
    handles.EKG=EKG;
    handles.EKG_laikai=DIA_duomenys.Data.Time_ECG';
    handles.EKG_Hz=EKG_Hz;
    handles.zmkl_lks=[]; handles.zmkl_pvd={};
    disp('Ieškoma R...');
    try   Aptikti_EKG_QRS_Callback2(hObject, eventdata, handles);
    ...
    setappdata(handles.figure1,'vardas',EEG.setname);
    %assignin('base','handles1',handles)
    handles=pirmieji_grafikai(hObject, eventdata, handles);
    %assignin('base','handles3',handles)
...
%susaldyk(hObject, eventdata, handles);
[f,p,i]=uigetfile({...
    '*.rrt;*.RRT' '*.RRT - Ritminukas'; ... % 1
    '*.txt;*.TXT' '*.TXT - QRS laikai'; ... % 2
    ... '*.dat;*.DAT' '*.DAT - R-R intervalai'; ...
    ... '*.txt;*.TXT' '*.TXT - R-R intervalai'; ...
    '*.txt;*.TXT' '*.TXT - EKG'; ... % 3
    ... '*.mat;*.MAT' '*.MAT - LabChart'; ...
    ... '*VNS_*.mat;*VNS_.*MAT' '*.MAT - KTU VNS'; ...
    ... 'DIA*.mat;DIA.*MAT' '*.MAT - KTU DIA'; ...
    ... 'AuriMod*.mat;AuriMod.*MAT' '*.MAT - KTU AuriMod'; ...
    '*.acq;*.ACQ' '*.ACQ - Biopac'; ... % 4
    '*.set;*.SET' '*.SET - EEGLAB'; ... % 5
    ... '*.edf;*.EDF' '*.EDF'; ...
    ... '*.cnt;*.CNT' '*.CNT - ASA Lab'; ...
    '*'           '*';... % 6
...
switch i
    case 1
        Importuoti_Ritminukas_RRT_Callback(hObject, eventdata, handles, rinkmena)
    case 2
    ...
    otherwise
        ...
        Importuoti_KTU_MAT_VNS_Callback(hObject, eventdata, handles, rinkmena);
        Importuoti_KTU_MAT_DIA_Callback(hObject, eventdata, handles, rinkmena);
        Importuoti_KTU_MAT_AuriMod_Callback(hObject, eventdata, handles, rinkmena);
        %}
end;
%guidata(handles.figure1,handles);
...
if isempty(R_laikai);
    ...
    if isempty(exp_tipas);
        exp_tipai={'R-R intervalai -> *.DAT' 'R laikai -> *.TXT' 'R laikai -> *.MAT' 'R laikai -> *.RRT' ' '};
        exp_tipas=listdlg(...
    ...
    switch exp_tipas
        ...
        case {3 'R laikai -> *.MAT' 'R laikai -> *.MAT (KTU)'}
            g='*.mat';
        case {4 'R laikai -> *.RRT' 'R laikai -> *.RRT (Ritminukas)'}
            g='*.rrt';
        case {5 'EEGLAB'}
            if ~get(handles.checkbox_ekg,'Value'); 
                susildyk(hObject, eventdata, handles); return; 
    ...
    try
        switch exp_tipas
            case {1 'RRI' ...
                ...
                dlmwrite(r, num2cell(d), 'precision',t, 'newline', 'pc') ;
            case {3 'R laikai -> *.MAT' 'R laikai -> *.MAT (KTU)'}
                R_laikai_taisyti=R_laikai/1000;
                save(r, 'R_laikai_taisyti','-mat','-append');
            case {4 'R laikai -> *.RRT'}
                papildomai_saugojimui={};
                R_laikai_taisyti=R_laikai/1000;
                if ~isempty(handles.EKG) && ~isempty(handles.EKG_laikai)
                    EKG=timeseries(handles.EKG(:), handles.EKG_laikai, 'Name', 'EKG');
                    papildomai_saugojimui=[papildomai_saugojimui {'EKG'}];
                end
                papildomai_saugojimui=[papildomai_saugojimui {'-mat'}];
                %if exist(r,'file')
                %    papildomai_saugojimui=[papildomai_saugojimui {'-append'}];
                %end
                save(r, 'R_laikai_taisyti', papildomai_saugojimui{:});
            case {5 'EEGLAB'}
                [~, EEG] = pop_newset([],[],[]);
...
msg=sprintf([...
    ...
    'Naudojate „Ritminukui“ adaptuotą „pop_RRI_perziura“ versiją. ' ...
    'Originali „pop_RRI_perziura“ platinama kaip EEGLAB papildinio „Darbeliai“ dalis.\n\n(c) 2014-2023 Mindaugas Baranauskas']);
msgbox(msg,[lokaliz('Apie') ' ' mfilename ],'help');
