...
if ismember('cause',fieldnames(err))
    for i=1:length(err.cause)
        msg=sprintf('%s\n\n%s\n\n%s\n',msg, err.cause{i}.identifier, err.cause{i}.message);
%         if ~isempty(err.cause{i}.stack)
%             for i=1:(length(err.cause{i}.stack)-1)
%                 msg=sprintf('%s\n> <a href="matlab:opentoline(''%s'',%d,1)">%s:%d</a>', msg, ...
%                     err.cause{i}.stack(i).file, ...
%                     err.cause{i}.stack(i).line, ...
%                     err.cause{i}.stack(i).name, ...
%                     err.cause{i}.stack(i).line ) ;
%             end
%             msg=sprintf('%s\n\n  %s\n\n',msg,err.cause{i}.stack(end).name);
%         end
    end
end
...
tekstas = [ [ 'MATLAB ' version ' ' OS ' ' lc]; ' '; '---------------'; tekstas ];
