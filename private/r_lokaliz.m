...
%
% Naudojimas:
%   lokalizuotas_tekstas = r_lokaliz(originalus_tekstas);
%
% Usage:
%   lokalizd_text = r_lokaliz(original_text);
%
...
function [txt,lokale]=r_lokaliz(orig_txt)
...
% create translation table example:
%{
data_file='r_localiz.mat';
...
save(data_file, 'LC_info','LC_TXT' ) ;
%}

try
   %LC_current_locale=struct('getLanguage','lt','getCountry','LT','getVariant','');
   %LC_current_locale=struct('getLanguage','en','getCountry','US','getVariant','');
   ...
   if isempty(LC_TXT) || isempty(LC_info)
      %disp('load locale');
      try
         load(fullfile(function_dir,data_file), 'LC_info','LC_TXT', '-mat');
      catch %err
   ...
   if isempty(LANG_id) || isequal(LC_TXT,{''}) %|| exist(data_file,'file') ~= 2
