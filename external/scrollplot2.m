...
function hScroll = addScrollPlot(hAx,axName)
    ...
    hPatch = patch(xlim([1,1,2,2]), ylim([1,2,2,1]), 'b', 'Parent',hScroll, 'FaceAlpha',.15, 'EdgeColor','w', 'EdgeAlpha',.15, 'ButtonDownFcn',@mouseDownCallback, 'tag','scrollPatch', 'userdata',axName);  %Note: FaceAlpha causes an OpenGL warning in Matlab 6
    ...
    hMenu = uicontextmenu(hFig); % add "hFig" to avoid "The UIContextMenu's parent figure is not the same as the parent figure of this object."
    ...
    try
        if isappdata(hFig, 'scrollplot_axisComponent')
            ...
            %if verLessThan('matlab','8')
                ...
                % MATLAB R2022b Warning: Cannot load an object of class 'proplistener'
                warning off MATLAB:load:classError
                % MATLAB R2022b Warning: An invalid default object has been detected while loading a heterogeneous array of class event.proplistener.
                % An empty array of class event.proplistener will be returned.
